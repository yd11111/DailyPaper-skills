...
async def _extract_first_page_async(pdf_path: Path, sem: asyncio.Semaphore) -> str:
    async with sem:
        return await asyncio.get_event_loop().run_in_executor(
            None, extract_first_page, pdf_path
        )
...
async def _build_entries(pdfs: list[Path], root: Path) -> list[dict]:
    sem = asyncio.Semaphore(10)
    texts = await asyncio.gather(*[_extract_first_page_async(p, sem) for p in pdfs])

    entries = []
    for pdf, text in zip(pdfs, texts):
...
def main() -> int:
    ...
    entries = asyncio.run(_build_entries(pdfs, root))
